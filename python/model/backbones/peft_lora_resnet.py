...
__all__ = ['ResNetLoRA', 'Resnet50LoRA']
...
# 在Bottleneck类前添加LoRA实现
class LoRAConv(nn.Module):
    def __init__(self, in_channels, out_channels, rank=8):
        super().__init__()
        # 参数维度调整为四维 [out, in, 1, 1]
        self.lora_A = nn.Parameter(jt.zeros((rank, in_channels, 1, 1)))
        self.lora_B = nn.Parameter(jt.zeros((out_channels, rank, 1, 1)))

        nn.init.gauss_(self.lora_A, 0.0, 0.02)
        nn.init.zero_(self.lora_B)

    def execute(self, x, original_conv):
        # 原始卷积输出
        original_out = original_conv(x)

        # 保持四维形状 [B, C, H*W, 1]
        B, C, H, W = x.shape
        x_reshaped = x.reshape(B, C, H * W, 1)  # 新增最后一个维度

        # 第一层低秩卷积 [B, rank, H*W, 1]
        lora_mid = jt.nn.conv2d(x_reshaped, self.lora_A, padding=0)

        # 第二层低秩卷积 [B, out_c, H*W, 1]
        lora_out = jt.nn.conv2d(lora_mid, self.lora_B, padding=0)

        # 恢复形状 [B, out_c, H, W]
        lora_out = lora_out.reshape(B, -1, H, W)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None,
                 groups=1, base_width=64, dilation=1, norm_layer=None, lora_rank=8):
        ...
        # LoRA适配层
        self.lora_conv = LoRAConv(width, planes * self.expansion, lora_rank)
        self.conv3 = conv1x1(width, planes * self.expansion)  # 保留原始卷积
    ...
    def execute(self, x):
        ...
        # 应用LoRA适配（自动处理4D形状）
        out = self.lora_conv(out, self.conv3)
        out = self.bn3(out)
...
#         self.stride = stride
#
#         self.lora = LoRAConv(width, planes * self.expansion, rank=8)
...
#
#         # 原始卷积 + LoRA调整
#         out = self.lora(out, self.conv3)  # 关键修改点
#         out = self.bn3(out)
#
#         if (self.downsample is not None):
#             identity = self.downsample(x)
...
class ResNetLoRA(nn.Module):
    ...
    def _freeze_most_params(self):
        for name, param in self.named_parameters():
            # print(name)
            if "lora" not in name:  # train lora only.
        ...
        if not self.merged:
            for m in self.modules():
                if isinstance(m, Bottleneck):
                    # 原始卷积权重 [out, in, 1, 1]
                    original_weight = m.conv3.weight.data
...
def _resnet(block, layers, **kwargs):
    model = ResNetLoRA(block, layers, **kwargs)
    return model
...
@BACKBONES.register_module()
def Resnet50LoRA(pretrained=False, **kwargs):
