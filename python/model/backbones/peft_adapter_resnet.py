...
__all__ = ['ResNetAdapter', 'Resnet50Adapter']
...
class Adapter(nn.Module):
    ...
    def execute(self, x):
        ...
        return out.reshape(B, H, W, C).permute(0, 3, 1, 2) + x  # 恢复原始形状
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1, base_width=64, dilation=1, norm_layer=None):
        ...
        self.adapter = Adapter(width * self.expansion)
...
class ResNetAdapter(nn.Module):
    ...
    def _freeze_most_params(self):
        for name, param in self.named_parameters():
            # print(name)
            if "adapter" not in name:  # train adapter only.
    ...
    def train(self):
        super(ResNetAdapter, self).train()
        self._freeze_stages()
...
def _resnet(block, layers, **kwargs):
    model = ResNetAdapter(block, layers, **kwargs)
    return model
...
@BACKBONES.register_module()
def Resnet50Adapter(pretrained=False, **kwargs):
