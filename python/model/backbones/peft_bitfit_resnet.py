...
__all__ = ['ResNetBitfit', 'Resnet50Bitfit']
...
class ResNetBitfit(nn.Module):
    ...
    def _freeze_most_params(self):
        for name, param in self.named_parameters():
            if "bias" not in name:  # train bias only.
...
def _resnet(block, layers, **kwargs):
    model = ResNetBitfit(block, layers, **kwargs)
    return model
...
@BACKBONES.register_module()
def Resnet50Bitfit(pretrained=False, **kwargs):
