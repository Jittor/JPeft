...
__all__ = ['ResNetPartial1', 'Resnet50Partial1']
...
class ResNetPartial1(nn.Module):
    ...
    def __init__(self, block, layers, return_stages=["layer4"],frozen_stages=-1,norm_eval=True,num_classes=None, groups=1, width_per_group=64, replace_stride_with_dilation=None, norm_layer=None):
        super(ResNetPartial1, self).__init__()
        if (norm_layer is None):
            norm_layer = nn.BatchNorm
    ...
    def _freeze_most_params(self):
        for name, param in self.named_parameters():
            if "layer4" not in name:  # train layer4 only.
    ...
    def train(self):
        super(ResNetPartial1, self).train()
        self._freeze_stages()
...
def _resnet(block, layers, **kwargs):
    model = ResNetPartial1(block, layers, **kwargs)
    return model
...
@BACKBONES.register_module()
def Resnet50Partial1(pretrained=False, **kwargs):
