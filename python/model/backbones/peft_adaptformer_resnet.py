...
__all__ = ['ResNetAdaptFormer', 'Resnet50AdaptFormer']
...
class AdaptFormer(nn.Module):
    def __init__(self, in_features, reduction=4):
        ...
        self.down = nn.Linear(in_features, in_features // reduction)
        self.act = nn.GELU()
        self.up = nn.Linear(in_features // reduction, in_features)
        self.scale = nn.Parameter(jittor.ones(1))

    def execute(self, x):
        # 处理4D特征图：B×C×H×W → B×(H×W)×C → 线性层 → 恢复形状
        B, C, H, W = x.shape
        x_flat = x.permute(0, 2, 3, 1).reshape(B * H * W, C)  # 转换为(B*H*W, C)
        out = self.down(x_flat)
        out = self.act(out)
        out = self.up(out) * self.scale

        # adapted = self.adapter(x_flat)  # 应用线性层
        return out.reshape(B, H, W, C).permute(0, 3, 1, 2)  # 恢复原始形状
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1, base_width=64, dilation=1, norm_layer=None):
        ...
        self.adaptformer = AdaptFormer(width * self.expansion)
    ...
    def execute(self, x):
        ...
        out = out1 + self.adaptformer(out2)
...
class ResNetAdaptFormer(nn.Module):
    ...
    def __init__(self, block, layers, return_stages=["layer4"],frozen_stages=-1,norm_eval=True,num_classes=None, groups=1, width_per_group=64, replace_stride_with_dilation=None, norm_layer=None):
        super(ResNetAdaptFormer, self).__init__()
        if (norm_layer is None):
            norm_layer = nn.BatchNorm
    ...
    def _freeze_most_params(self):
        for name, param in self.named_parameters():
            # print(name)
            if "adaptformer" not in name:  # train adapter only.
    ...
    def train(self):
        super(ResNetAdaptFormer, self).train()
        self._freeze_stages()
...
def _resnet(block, layers, **kwargs):
    model = ResNetAdaptFormer(block, layers, **kwargs)
    return model
...
@BACKBONES.register_module()
def Resnet50AdaptFormer(pretrained=False, **kwargs):
