...
__all__ = ['ResNetFixed', 'Resnet50Fixed']
...
class ResNetFixed(nn.Module):
    ...
    def _freeze_most_params(self):
        for name, param in self.named_parameters():
            print(name)
            if "peft" not in name and "downstream_norm" not in name:  # train LN only.
...
def _resnet(block, layers, **kwargs):
    model = ResNetFixed(block, layers, **kwargs)
    return model
...
@BACKBONES.register_module()
def Resnet50Fixed(pretrained=False, **kwargs):
