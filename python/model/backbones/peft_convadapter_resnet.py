...
__all__ = ['ResNetConvAdapter', 'Resnet50ConvAdapter']
...
class ConvAdapter(nn.Module):
    ...
    def __init__(self, inplanes, outplanes, width,
        ...
        self.se = nn.Parameter(1.0 * jt.ones((1, outplanes, 1, 1)), requires_grad=True)
        # self.se = 4.0
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1, base_width=64, dilation=1, norm_layer=None):
        ...
        self.convadapter = ConvAdapter(width, planes * self.expansion,
                                             kernel_size=3,
                                             padding=1,
                                             width=32,
                                             stride=stride,
...
class ResNetConvAdapter(nn.Module):
    ...
    def __init__(self, block, layers, return_stages=["layer4"],frozen_stages=-1,norm_eval=True,num_classes=None, groups=1, width_per_group=64, replace_stride_with_dilation=None, norm_layer=None):
        super(ResNetConvAdapter, self).__init__()
        if (norm_layer is None):
            norm_layer = nn.BatchNorm
    ...
    def train(self):
        super(ResNetConvAdapter, self).train()
        self._freeze_stages()
...
def _resnet(block, layers, **kwargs):
    model = ResNetConvAdapter(block, layers, **kwargs)
    return model
...
@BACKBONES.register_module()
def Resnet50ConvAdapter(pretrained=False, **kwargs):
